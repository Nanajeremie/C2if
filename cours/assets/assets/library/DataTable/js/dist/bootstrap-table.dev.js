...
(function ($) {
  ...
  var compareObjects = function compareObjects(objectA, objectB, compareLength) {
    ...
    for (var i = 0; i < objectAProperties.length; i++) {
      propName = objectAProperties[i]; // If the property is not in the object B properties, continue with the next property
  ...
  BootstrapTable.DEFAULTS = {
    ...
    iconsPrefix: 'fas',
    // glyphicon of fa (font awesome)
    icons: {
      paginationSwitchDown: 'fa-caret-square-down',
      paginationSwitchUp: 'fa-caret-square-up',
      refresh: 'fa-sync-alt',
      toggle: 'fa-clipboard-list',
      columns: 'fa-th',
      detailOpen: 'fa-plus',
  ...
  BootstrapTable.prototype.initTable = function () {
    ...
    this.$el.find('>tbody>tr').each(function (y) {
      ...
      $(this).find('>td').each(function (x) {
        ...
        row[field] = $(this).html(); // save td's id, class and data-* attributes
  ...
  BootstrapTable.prototype.initToolbar = function () {
    ...
    if (typeof this.options.toolbar === 'string' || _typeof(this.options.toolbar) === 'object') {
      $(sprintf('<div class="bs-bars col-lg-3 col-md-3 col-sm-6"></div>')).appendTo(this.$toolbar).append($(this.options.toolbar));
    }

    if (this.options.search) {
      html = [];
      html.push('<div class="col-lg-4 col-md-3 col-sm-6' + ' search">', sprintf('<input class="form-control' + sprintf(' input-%s', this.options.iconSize) + '" type="text" placeholder="%s">', this.options.formatSearch()), '</div>');
    ...
    html = [sprintf('<div class="pagination columns columns-%s btn-group col-lg-4 col-md-4 col-sm-6 ">', this.options.buttonsAlign, this.options.buttonsAlign)];
    ...
    if (this.options.showPaginationSwitch) {
      html.push(sprintf('<button class="border btn' + sprintf(' btn-%s', this.options.buttonsClass) + sprintf(' btn-%s', this.options.iconSize) + '" type="button" name="paginationSwitch" aria-label="pagination Switch" title="%s">', this.options.formatPaginationSwitch()), sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.paginationSwitchDown), '</button>');
    }

    if (this.options.showRefresh) {
      html.push(sprintf('<button class="border btn' + sprintf(' btn-%s', this.options.buttonsClass) + sprintf(' btn-%s', this.options.iconSize) + '" type="button" name="refresh" aria-label="refresh" title="%s">', this.options.formatRefresh()), sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.refresh), '</button>');
    }

    if (this.options.showToggle) {
      html.push(sprintf('<button class="border btn' + sprintf(' btn-%s', this.options.buttonsClass) + sprintf(' btn-%s', this.options.iconSize) + '" type="button" name="toggle" aria-label="toggle" title="%s">', this.options.formatToggle()), sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.toggle), '</button>');
    }

    if (this.options.showColumns) {
      html.push(sprintf('<div class="keep-open btn-group" title="%s">', this.options.formatColumns()), '<button type="button" aria-label="columns" class="border btn' + sprintf(' btn-%s', this.options.buttonsClass) + sprintf(' btn-%s', this.options.iconSize) + ' dropdown-toggle" data-toggle="dropdown">', sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.columns), ' <span class="caret"></span>', '</button>', '<ul class="dropdown-menu" role="menu">');
  ...
  BootstrapTable.prototype.initRow = function (item, i, data, parentDom) {
    ...
    $.each(this.header.fields, function (j, field) {
      ...
      style = sprintf('style="%s"', csses.concat(that.header.styles[j]).join('; ')); // handle td's id and class
  ...
  BootstrapTable.prototype.initBody = function (fixedScroll) {
    ...
    this.$body.find('> tr[data-index] > td').off('click dblclick').on('click dblclick', function (e) {
      ...
      that.trigger(e.type === 'click' ? 'click-row' : 'dbl-click-row', item, $tr, field); // if click to select - then trigger the checkbox/radio click
  ...
  BootstrapTable.prototype.load = function (data) {
    var fixedScroll = false; // #431: support pagination
